...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    ...
    await request(server).get('/api/wcd/todos').expect(200, {
      items: [],
    });
    ...
    await request(server)
      .get(`/api/wcd/todos/${createdTodoItem.id}`)
  ...
  it('should forward errors from the TodoListService', async () => {
    ...
    const listRes = await request(server).get('/api/wcd/todos');
    expect(listRes.status).toBe(401);
    expect(listRes.body).toMatchObject({
      error: { name: 'AuthenticationError' },
    });

    const getRes = await request(server).get('/api/wcd/todos/123');
